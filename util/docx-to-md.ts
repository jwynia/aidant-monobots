// deno run --allow-read --allow-write --allow-net --allow-env --allow-run docx-to-md.ts
// deno install --allow-read --allow-write --allow-net --allow-env --allow-run --global --name docx2md docx-to-md.ts
...
if (args.help || (!args.input && !args["install-deps"])) {
  ...
  Usage:
    deno run --allow-read --allow-write --allow-net --allow-env --allow-run docx-to-md.ts -i <input.docx> -o [output.md] [-d]

  Options:
    -i, --input         Input DOCX file (required unless --install-deps is used)
    -o, --output        Output Markdown file (default: same name as input with .md extension)
...
// Process DOCX to Markdown
async function convertDocxToMarkdown(inputPath: string, outputPath: string, debugMode: boolean): Promise<void> {
  try {
    ...
    console.log(`Checking input DOCX file: ${inputPath}`);
    ...
    // Create a temporary output path for HTML (if debug mode is enabled)
    let tempHtmlPath: string | null = null;
    if (debugMode) {
      tempHtmlPath = `${Deno.makeTempFileSync()}.html`;
      console.log(`Will save intermediate HTML to: ${tempHtmlPath}`);
    }
    
    // Create a temporary output path for the Markdown
    const tempOutputPath = `${Deno.makeTempFileSync()}.md`;
    ...
    // If debug mode is enabled, also generate HTML
    if (debugMode && tempHtmlPath) {
      // First convert DOCX to HTML for debugging
      const htmlCommand = new Deno.Command("pandoc", {
        args: [
          "-f", "docx",
          "-t", "html",
          "-o", tempHtmlPath,
      ...
      if (htmlResult.code !== 0) {
        const errorOutput = new TextDecoder().decode(htmlResult.stderr);
        console.error("Pandoc HTML conversion error:", errorOutput);
        console.warn("Continuing with Markdown conversion...");
      } else {
        console.log(`Debug HTML file saved to: ${tempHtmlPath}`);
      }
    }
    
    // Execute the Pandoc command for Markdown conversion
    ...
    try {
      ...
      const markdownData = await Deno.readFile(tempOutputPath);
      
      // Write to the final destination
      await Deno.writeFile(outputPath, markdownData);
      
      // Remove the temporary output file
      await Deno.remove(tempOutputPath);
      
      console.log(`Markdown file successfully created at: ${outputPath}`);
    } catch (error) {
...
async function main() {
  ...
  const outputFile = args.output || inputFile.replace(/\.docx$/i, ".md");
  const debugMode = !!args.debug;

  // Run the conversion
  await convertDocxToMarkdown(inputFile, outputFile, debugMode);
}
